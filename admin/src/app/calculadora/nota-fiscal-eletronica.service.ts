...
import SolicitacaoNofitificacaoProduto from '../models/solicitacaoNotificacaoProduto';
...
export class NotaFiscalEletronicaService {
  ...
  carregarSolicitacoes(){
    if( this.produtos != undefined && this.produtos.length > 0 ){
      let consultas = [];
      for(let i = 0; i < this.produtos.length; i++ ){
        consultas.push(SolicitacaoNofitificacaoProduto.getAll(new Query("codigoProduto", "==", this.produtos[i].codigo)));
      }
        ...
        if(input.files.length > 0){
          ...
          reader.onload = () => {
            ...
            for (let i = 0; i < detalhamentosProdutos.length; i++) {
              ...
              let codigo = produto.getElementsByTagName("cProd")[0].textContent;
